...
class MoveArm:
    ...
    def check_arm_move_done_and_publish( self ):
        if self.completion_message != "":
            is_done = ( self.target_base       == self.TARGET_IGNORE_VALUE() ) and \
                      ...
                      ( self.target_elbow2     == self.TARGET_IGNORE_VALUE() ) and \
                      ( self.target_wrist      == self.TARGET_IGNORE_VALUE() ) 
    ...
    def validate_state_wrist_controller( self, state ):
        self.target_wrist = self.validate_state_generic( state, self.target_wrist, self.wrist_command )
        self.check_arm_move_done_and_publish()
